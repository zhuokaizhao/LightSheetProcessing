...
void Anim::build_video()
{
    ...
    for(std::string type: {"max", "avg"})
    {
        ...
        cv::VideoWriter vw(out_file.c_str(), cv::VideoWriter::fourcc('F', 'F', 'V', '1'), 10.0, s, true);
        ...
        for(int i = 0; i < tmax; i++)
        {
            ...
            putText(curImage, frameNum, cv::Point2f(20, 20), cv::FONT_HERSHEY_PLAIN, 2, Scalar(0,0,255,255), 1, 8, bottomLeftOrigin=true )
            vw << curImage;
        }
        vw.release();
    }
